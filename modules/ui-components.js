...
export const UIComponents = {
    ...
    createUI() {
        if (this.floatingUICreated && this.panel) return this.panel;
    ...
    setupAskModeEventHandlers() {
        if (!this.askTextInput || !this.askButton || !this.attachImageButton) return;
    ...
    showMessage(message, isError = false) {
        if (window.CaptureAI.UICore?.showMessage) window.CaptureAI.UICore.showMessage(message, isError);
    },

    handleAskQuestion(question, imageData = null) {
        if (window.CaptureAI.UICore?.handleAskQuestion) window.CaptureAI.UICore.handleAskQuestion(question, imageData);
    },

    ensureAskModeExists() {
        if (!this.askModeContainer) this.attachAskMode();
