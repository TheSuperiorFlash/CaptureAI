...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  elements.getFreeKeyBtn.addEventListener('click', handleGetFreeKey);
  ...
  /**
   * Handle get free key request
   */
  async function handleGetFreeKey() {
    elements.getFreeKeyBtn.disabled = true;
    elements.getFreeKeyBtn.textContent = 'Generating...';

    try {
      const licenseKey = await AuthService.requestFreeKey();

      // Show the key in the input
      elements.licenseKeyInput.value = licenseKey;

      showResponseMessage('Free license key created and activated!', 'success');
      ...
      showResponseMessage(error.message || 'Failed to create free key', 'error');
      elements.getFreeKeyBtn.disabled = false;
      elements.getFreeKeyBtn.textContent = 'Get Free Key (10/day)';
    }
  }
