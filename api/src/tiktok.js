...
const DEFAULT_PIXEL_ID = 'D70T45BC77U98PN6KECG';
// Keeps webhook handling bounded if TikTok is slow
const TIKTOK_FETCH_TIMEOUT_MS = 5000;

/**
 * Send a server-side event to TikTok Events API.
 * @param {object} env - Cloudflare Worker env (must have TIKTOK_EVENTS_TOKEN, optional TIKTOK_PIXEL_ID)
...
export async function sendTikTokEvent(env, eventName, payload) {
  try {
    ...
    if (!email) {
      console.error('TikTok Events API: missing email, skipping event', eventName);
      return;
    }

    const pixelCode = env.TIKTOK_PIXEL_ID || DEFAULT_PIXEL_ID;
    ...
    const res = await fetchWithTimeout(TIKTOK_EVENTS_URL, {
      method: 'POST',
      headers: {
